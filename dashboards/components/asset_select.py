# dashboards/components/asset_select.py
    ...
    *,
    ...
    with st.sidebar:
        ...
        if assets:
            ...
            with col_load:
                if st.button("Load asset", type="primary", use_container_width=True, key=f"{key_prefix}_load"):
                    inst = id_to_inst.get(selected_id)
                    # Persist loaded selection
                    st.session_state[loaded_id_key] = int(selected_id) if selected_id is not None else None
