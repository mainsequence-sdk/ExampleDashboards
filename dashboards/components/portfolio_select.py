...
def _search_portfolios(q: str) -> List[Dict[str, Any]]:
    """
    Search MainSequence PortfolioIndexAsset by current_snapshot.name (contains).
    Returns a list of {'id', 'name', 'instance'} dicts (safe for UI rendering).
    """
    ...
    for p in (results or []):
        try:
            out.append({"id": p.id, "name": p.current_snapshot.name, "instance": p})
        except Exception:
            pass
    return out
